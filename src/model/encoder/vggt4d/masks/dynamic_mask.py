...
def extract_dyn_map(qk_dict: dict, images: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        qk_dict: Dictionary containing 'global_tok_q', 'global_tok_k', 'global_cam_q'
        images: Input images tensor with shape [S, C, H, W] or [B, S, C, H, W]

    Returns:
        Dynamic maps tensor with shape [S, H//14, W//14]
    """
    dyn_maps = []
...
def batch_extract_dyn_map(qk_dict: dict, images: torch.Tensor) -> torch.Tensor:
    """Batch extraction of dynamic maps for memory efficiency.

    Args:
        qk_dict: Dictionary containing Q/K tensors
